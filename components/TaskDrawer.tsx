...
export default function TaskDrawer({ isOpen, onClose, task }: TaskDrawerProps) {
      ...
      <SheetContent className="w-full sm:max-w-md overflow-y-auto">
        ...
        <Tabs defaultValue="details" className="mt-4" onValueChange={setActiveTab}>
          <TabsList className="grid grid-cols-3">
            ...
            <TabsTrigger value="comments">Comments ({task.comments})</TabsTrigger>
            <TabsTrigger value="subtasks">Subtasks ({task.subtasks})</TabsTrigger>
          </TabsList>
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {task.assignees.map((assigneeId) => (
                  <div key={assigneeId} className="flex items-center bg-gray-100 rounded-full px-2 py-1">
                    <Avatar className="h-5 w-5 mr-1">
                      <AvatarFallback>{assigneeId.charAt(0)}</AvatarFallback>
                    </Avatar>
                    <span className="text-xs">User {assigneeId}</span>
                  </div>
                ))}
              </div>
            </div>
                  ...
                  <div key={i} className="bg-gray-50 p-3 rounded-lg">
                    <div className="flex items-center mb-2">
                      <Avatar className="h-6 w-6 mr-2">
                        <AvatarFallback>{task.assignees[i % task.assignees.length]?.charAt(0) || 'U'}</AvatarFallback>
                      </Avatar>
                      <div>
                        <div className="text-xs font-medium">User {task.assignees[i % task.assignees.length] || 'Unknown'}</div>
                        <div className="text-xs text-gray-500">Apr {i + 1}, 2025</div>
                      </div>
                    </div>
              ...
              <div className="space-y-2">
                {[...Array(task.subtasks)].map((_, i) => (
                  <div key={i} className="flex items-center justify-between p-2 border rounded">
                    <div className="flex items-center">
                      <CheckCircle className={`h-4 w-4 mr-2 ${i < 2 ? "text-green-500" : "text-gray-300"}`} />
